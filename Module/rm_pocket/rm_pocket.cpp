...
bool rmPocket::onFrameComplete(const CRSF_broadcast_frame_t &frame) {
    ...
    auto trimTrans = [fuzzyEqual100](uint16_t ch_value) -> RM_Trim_State_t {
        if (fuzzyEqual100(ch_value, 1760)) {
            return RM_Trim_State_t::UP;
        } else if (fuzzyEqual100(ch_value, 1401)) {
            return RM_Trim_State_t::DOWN;
        } else if (fuzzyEqual100(ch_value, 222)) {
            return RM_Trim_State_t::LEFT;
        } else if (fuzzyEqual100(ch_value, 583)) {
            return RM_Trim_State_t::RIGHT;
        } else {
            return RM_Trim_State_t::MIDDLE; //默认返回中立
        }
    };
    ...
    {       
        ...
        rm_state_.joyRHori = ch[0];
        rm_state_.joyRVert = ch[1];
        rm_state_.joyLVert = ch[2];
        rm_state_.joyLHori = ch[3];

        rm_state_.swE = ch[4] > 1500 ? RM_2_POS_SW_State_t::DOWN : RM_2_POS_SW_State_t::UP;

        rm_state_.x_cnt = cntTrans(ch[5]);
        rm_state_.y_cnt = cntTrans(ch[6]);
        rm_state_.cursor = (ch[7] > 1500 ? 2 : (ch[7] < 500 ? 0 : 1));

        rm_state_.swB = (fuzzyEqual35(ch[8], 172) ? RM_3_POS_SW_State_t::UP : (fuzzyEqual35(ch[8], 992) ? RM_3_POS_SW_State_t::MIDDLE : RM_3_POS_SW_State_t::DOWN));
        rm_state_.swC = (fuzzyEqual35(ch[9], 172) ? RM_3_POS_SW_State_t::UP : (fuzzyEqual35(ch[9], 992) ? RM_3_POS_SW_State_t::MIDDLE : RM_3_POS_SW_State_t::DOWN));

        rm_state_.swA = ch[10] > 1500 ? RM_2_POS_SW_State_t::DOWN : RM_2_POS_SW_State_t::UP;
        rm_state_.swD = ch[11] > 1500 ? RM_2_POS_SW_State_t::DOWN : RM_2_POS_SW_State_t::UP;
        rm_state_.pot = ch[12];

        rm_state_.trimLeft = trimTrans(ch[13]);
        rm_state_.trimRight = trimTrans(ch[14]);
