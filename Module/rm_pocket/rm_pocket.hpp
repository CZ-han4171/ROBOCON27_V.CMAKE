...
//按钮
enum class RM_BTN_State_t : uint8_t {
    RELESED,
    PRESSED,
};
...
//两段开关
enum class RM_2_POS_SW_State_t : uint8_t {
    UP,
    DOWN
};
...
//三段开关
enum class RM_3_POS_SW_State_t : uint8_t {
    UP, //后端按下，前端翘起
    MIDDLE,
    DOWN
};

enum class RM_Trim_State_t : uint8_t {
    UP,
    DOWN,
    LEFT,
    RIGHT,
...
public:
    ...
    void init(void) {
        std::memset(&rm_state_, 0, sizeof(rm_state_));
        resetState();
    };

    bool processByte(uint8_t byte);

    const RM_state_t &getRCState() const {
        return rm_state_;
    }
