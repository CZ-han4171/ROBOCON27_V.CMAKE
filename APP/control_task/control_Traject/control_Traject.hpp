...
public:
    ...
    void Run(pub_chassis_cmd Now) {
        ...
        float_t cos_yaw = cosf(now.yaw*kDegToRad);
        float_t sin_yaw = sinf(now.yaw*kDegToRad);
...
private:
    ...
    void Update_TarjSpeed(){
        ...
        v_Acc = v_output + Traject_speed.Acc_linear*dt;
        v_Dec = sqrt(2.0f*Traject_speed.Dec_linear*(L - s_now));

        if(Traject_speed.v_Max < v_Acc && Traject_speed.v_Max < v_Dec){
            v_output = Traject_speed.v_Max;
        }else if(v_Acc > v_Dec){
            v_output = v_Dec;
        }else {
            v_output = v_Acc;
        ...
        if(fabsf(dyaw_ds)*kDegToRad*v_output > Traject_speed.w_Max && fabsf(dyaw_ds) > 0.01f){
            v_output = Traject_speed.w_Max/(fabsf(dyaw_ds)*kDegToRad);
        }
    ...
    speed_data Traject_speed{1.2f,0.9f,2.1f,M_PI*0.6f};
